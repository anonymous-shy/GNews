...
object ZookeeperHelper {
  ...
  def loadOffsets(topicSet: Set[String], defaultOffset: Map[TopicAndPartition, Long],prefix:String): Map[TopicAndPartition, Long] = {
    val kafkaOffsetPath = s"/streaming/kafka_offsets/${prefix}full"
  ...
  def storeOffsets(offsets: Map[TopicAndPartition, Long],prefix:String): Unit = {
    val kafkaOffsetPath = s"/streaming/kafka_offsets/${prefix}full"
