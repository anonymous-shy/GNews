    ...
    while (true) {
      ...
      for (map <- maps) {
        breakable {
          ...
          val maxTimeStamp = if (getMaxTs != null) {
            getMaxTs.toString.replace(' ', 'T')
          } else {
          ...
          val query = if ("article".equals(doc_type) || "gallery".equals(doc_type) || "video".equals(doc_type)) {
            s"""
               |{
               |  "query":{
               |    "constant_score":{
          ...
          val queryRdd = sc.esRDD(s"${read_index}_$yearMonth/$doc_type", query).coalesce(3).persist()
          val queryCnt = queryRdd.count
          if (queryCnt == 0) break
          val errCnt: Accumulator[Int] = queryRdd.sparkContext.accumulator(0, "Error Record")
          val rightCnt: Accumulator[Int] = queryRdd.sparkContext.accumulator(0, "Right Record")
          val pCnt: Accumulator[Int] = queryRdd.sparkContext.accumulator(0, "Process Record")
          val resRdd = checkGNewsValid(queryRdd, pCnt, rightCnt, errCnt)
